...
class Budut extends Db{
  ...
  function DelRedisAll(){
    try { $this->RedisKoneksi()->flushall(); $status = 1; }catch (Exception $e) { $status = 0; } 
  }
  function RedisDelKey($name,$id){
    try { $this->RedisKoneksi()->del($name.$id); }catch (Exception $e) { } 
  }
  //cache redis ---------------------
...
//&formnilai.php -------------------------------------
  ...
  function form_nilai($id_mapel,$id_kelas,$id_sesi,$id_ruang){
    ...
    $where ="WHERE nilai.id_mapel='$id_mapel' AND kelas.id_kelas='$id_kelas'";
    ...
    FROM nilai
      INNER JOIN siswa ON siswa.id_siswa=nilai.id_siswa
      ...
      INNER JOIN ujian ON ujian.id_ujian = nilai.id_ujian
...
//&leger.php ---------------------------------
  function id_mapel($id){ //tampilkan pengawas berdasakan id 
    $sql="SELECT DISTINCT id_mapel FROM nilai WHERE id_ujian='$id'";
    $log=$this->con->query($sql) or die($this->con->error);
  ...
  function v_nilai2($id,$id_mapel){
    $sql="SELECT nilai_esai,skor FROM nilai WHERE id_siswa='$id' AND id_mapel='$id_mapel'";
    $log=$this->con->query($sql) or die($this->con->error);
     $id2=mysqli_fetch_array($log);
    return $id2;
  }
...
//&anso ----------analisa soal
  ...
  function v_nilai($id,$kls,$jrs){
    ...
    $sql="SELECT id_nilai,id_mapel,nilai.id_siswa,id_ujian,kode_ujian,nilai_esai,skor,id_kelas,idpk FROM nilai INNER JOIN siswa ON nilai.id_siswa=siswa.id_siswa $where";
    $log=$this->con->query($sql) or die($this->con->error);
    return $log;
  }
  ...
  function nilairata2($id_mapel){
    $sql = "SELECT ROUND(((SUM(skor)+SUM(nilai_esai))/COUNT(id_nilai)),2) AS jml_nilai FROM nilai WHERE id_mapel='$id_mapel'";
    $log=$this->con->query($sql) or die($this->con->error);
  ...
  //melakukan pencocokan jawaban benar pada semua siswa yg sudah ujian
    $sql = "SELECT id_siswa,id_soal,jawaban FROM nilai WHERE id_mapel='$id_mapel'";
    $log=$this->con->query($sql) or die($this->con->error);
  ...
  function nilai_rata2($id_mapel){
    $sql = "SELECT ROUND(((SUM(skor)+SUM(nilai_esai))/COUNT(id_nilai)),2) AS jlh_siswa FROM nilai WHERE id_mapel='$id_mapel'";
    $log=$this->con->query($sql) or die($this->con->error);
  ...
  function nilai_ranking($idmapel,$kls,$jrs){
    if(!empty($kls)){
      $where = "WHERE nilai.id_mapel='$idmapel' and id_kelas='$kls'";
    }
    else{
      $where = "WHERE nilai.id_mapel='$idmapel' and idpk='$jrs'";
    }
    
    $sql = "SELECT *,siswa.nama AS nama_siswa FROM nilai INNER JOIN siswa ON nilai.id_siswa=siswa.id_siswa INNER JOIN ujian ON nilai.id_ujian=ujian.id_ujian $where  ORDER BY CAST((skor+nilai_esai) AS DECIMAL) DESC";
    $log=$this->con->query($sql) or die($this->con->error);
    ...
    function siswa_tidak_ujian($kls,$jrs,$ujian=null){
      ...
      if($ujian==null){
        ...
        foreach ($log as $value) {
          ...
          $sql2="SELECT * from nilai INNER JOIN ujian ON ujian.id_ujian=nilai.id_ujian where nilai.id_siswa='$idsiswa' and nilai.id_mapel='$ujian'";
          $log2=$this->con->query($sql2) or die($this->con->error);
...
//&nilai function  Nilai2.php
    function Tampil_nilai2(){ //tampil siswa
      ...
      $id_mapel = $_GET['id'];
      if(empty($id_jrs)){
        $sql = "SELECT * FROM siswa a join nilai b on a.id_siswa=b.id_siswa WHERE id_kelas='$id_kls' and id_ujian='$id_mapel'";
      }
      elseif(empty($id_kls)){
        $sql = "SELECT * FROM siswa a JOIN nilai b ON a.id_siswa=b.id_siswa WHERE idpk='$id_jrs' AND id_ujian='$id_mapel'";
      }
      else{
    ...
    function Tampil_nilai3($id_siswa){ //tampil siswa beserta nilai benar salah
      ...
      $id_mapel = $_GET['id'];
      if(empty($id_jrs)){
        $sql ="SELECT * FROM nilai JOIN siswa ON siswa.id_siswa=nilai.id_siswa JOIN kelas ON siswa.id_kelas = kelas.id_kelas WHERE siswa.id_kelas='$id_kls' and id_ujian='$id_mapel' and siswa.id_siswa='$id_siswa'";
      }
      elseif(empty($id_kls)){
        $sql ="SELECT * FROM nilai JOIN siswa ON siswa.id_siswa=nilai.id_siswa JOIN kelas ON siswa.id_kelas = kelas.id_kelas WHERE siswa.idpk='$id_jrs' and id_ujian='$id_mapel' and siswa.id_siswa='$id_siswa'";
      }
      else{
...
//Bot Telegram --------------------------------
 ...
 //Absen Siswa Mapel Manual untuk Guru --------------------------------

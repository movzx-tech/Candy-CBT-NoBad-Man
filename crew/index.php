...
<body class='hold-transition skin-green-light sidebar-mini fixed <?= $sidebar ?>'>
		...
		<div class='content-wrapper' style='background-image: url(backgroun.jpg);background-size: cover;'>
			<section class='content-header' style="<?= $style; ?>" >
				...
				<div style='float:right; margin-top:-37px;'>
					<button id="clear_redis" class='btn  btn-flat  bg-aqua'><i class='fa fa-trash'></i> Clear Cache Redis</button>
					...
					<?php if ($ac == '') : ?>
						<div class='row'>
							<div class='col-md-12'>
								...
								<div class='box box-solid'>
									<div class='box-header with-border'>
										<h3 class='box-title'>Status Peserta </h3> &nbsp;(Status Peserta Bisa Bisa Juga Di lihat Di Menu Status Ujian -> Status)
										<div class='box-tools pull-right '>
											...
											<button id="tbl_selesai" class="btn btn-success"><i class="fa fa-check"></i> Tampil Paksa Tombol Selesai</button>
											<button id="tbl_selesai2" class="btn btn-danger"><i class="fa fa-times"></i> Kunci Tombol Selesai</button>
										</div>
									</div><!-- /.box-header -->
										...
										<div class='modal-dialog'>
											<div class='modal-content'>
												...
												<div class='modal-body'>
													<p>
													Agar Berjalan Pastikan Halaman Status Peserta Aktif atau Di Buka ^_^
													<hr>
													Tombol Selesai Akan tampil jika waktu yang di setting Tombol Selesai >= dengan waktu server berjalan (Biar keluar secara serempak ^_^)
													<hr>
													Tampil Tombol Selesai Bisa juga di atur Secara Manual dan Bisa di Kunci Secara Manual juga.<br>
													Labih optimal Jika setting tombol selesainya di kosongkan saja (jangan di isi apa apa), nanti satatus akan null X (artinya manual semua)
													<hr>
													Jika Waktu Sudah memasukin tombol selesai tampil, maka kunci tombol selesai tidak akan berfungsi ^_^ 
													</p>
												</div>
											</div>
										</div>
									...
									<div class='box-body'>
										...
										<div>Jika Mengarah Ke Bawah Terus, Bisa di Refres<div style="float: right">Kunci Tombol Selesai Akan Berfungsi maksimal Jika Tombol Selesai Di Kosongkan</div></div>
	...
	<script>
		...
		let status_siswa=$('#tablestatus2').DataTable({
			...
			"ajax": "<?= $homeurl ?>/<?= $crew; ?>/statuspeserta2.php<?= $a ?>",
			processing: true,
			deferLoading:0,
		...
		$(document).on('click', '#clear_redis', function() { //hapus cache redis
			...
			$.ajax({
				...
				success: function(response) {
					...
					// if(response == 1){
					// 	alert("Cache Berhasil Di Hapus");
					// }
					// else{
					// 	alert('Gagal Hapus Cache');
					// }
				}
			});
		});
